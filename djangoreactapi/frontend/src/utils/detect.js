 ...
 */
export const detectImage = async (imgSource, model, classThreshold, canvasRef, setLabelValue) => {
  ...
  await model.net.executeAsync(input).then((res) => {
    ...
    renderBoxes(canvasRef, classThreshold, boxes_data, scores_data, classes_data, [xRatio, yRatio], setLabelValue); // render boxes
 ...
 * @param {HTMLCanvasElement} canvasRef canvas reference
 */
export const detectVideo = (vidSource, model, classThreshold, canvasRef, setLabelValue) => {
  const [modelWidth, modelHeight] = model.inputShape.slice(1, 3); // get model width and height
